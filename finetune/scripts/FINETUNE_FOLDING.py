...
# ============================================================
# HARDWARE DETECTION -- ported from pipeline_crunching/RAG_FOLDING.py's already-unified,
# already-validated cross-platform probing rather than re-merging the three finetune clients'
# own copies verbatim: the linux finetune client had no Intel GPU check at all (Windows did),
# and its AMD check was a flat "found an AMD GPU -> assume 16GB" guess with no discrete-vs-
# integrated distinction (Windows/RAG_FOLDING both estimate integrated GPUs from a RAM fraction
# instead). Reusing the more accurate, already cross-platform-tested implementation closes both
# gaps instead of carrying them forward into a second file.
# ============================================================
...
def choose_model() -> tuple:
    ...
    # RAG-crunching client's -- no 3b tier, and no CPU-only fallback tier either.
    if vram >= 20.0:
...
def main():
    print(f"=== Cubyz Fine-Tune Dataset Generation Node ({PLATFORM_LABELS[PLATFORM]} Edition) ===")
