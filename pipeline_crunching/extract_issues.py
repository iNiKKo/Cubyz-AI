"""
Fetches GitHub Issue discussions (not PRs) from PixelGuys/Cubyz into
raw_cubyz_dataset/reviews/issues.json -- for the same purpose as extract_reviews.py (behavioral/
judgment training material) but a different slice of it. PR reviews teach "is this code change
good and why" (code-quality judgment). Issues teach "here's a reported problem, diagnose it" --
symptom -> hypothesis -> root cause -> resolution, the same shape as the "Debugging help"
behavior already called out in webapp/local_rag_chat.py's system prompt.

Restored from archive/prototype_2_review_extraction/ (the original generator behind the
github_issue_*_discussion entries already in knowledge_base/reviews/ -- confirmed by an exact
1007/1007 chunk_id match) with paths anchored to this project's actual layout, and pointed at
raw_cubyz_dataset/reviews/issues.json so re-running this extends what's already there.

Different content shape than reviews, so this stays its own script rather than a flag on
...
server_textual.py's rag_initialize_chunks() treats ANY .json file inside a "reviews"-named
top-level folder as a pre-chunked review-shaped array. No pipeline changes needed.

Usage:
    export GITHUB_TOKEN=...   # strongly recommended -- issue+comment fetching is call-heavy
    python3 pipeline_crunching/extract_issues.py
"""
import json
import os
import time
...
OUTPUT_FILE = os.path.join(PIPELINE_ROOT, "raw_cubyz_dataset", "reviews", "issues.json")
...
# cheap and safe.
STATE_FILE = os.path.join(PIPELINE_ROOT, "campaign_state", "processed_issues.txt")
