...
def ensure_ollama_running_and_model_pulled(model_name):
    try: urllib.request.urlopen("http://localhost:11434", timeout=3)
    except Exception:
        if shutil.which("ollama"): subprocess.Popen(["ollama", "serve"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        else: fix_docker_permissions(); subprocess.run(["docker", "start", "ollama"], capture_output=True, check=True)
        time.sleep(5)
...
def main():
    ...
    print("=== Cubyz Dataset Distributed Folding Node (Linux Edition) ===")
