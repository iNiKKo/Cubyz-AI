...
# The dashboard's live-redrawing terminal display -- see _dashboard_loop's own comment for why
# this replaced four different hand-rolled ANSI redraw attempts. `pip install rich` if missing.
...
def _build_dashboard_lines() -> list:
    """Builds the whole admin console as a set of per-user panels (status/hardware/lane/joined/
    ...
    Deliberately unchanged from the pre-rich version except for its name and that it now RETURNS
    the lines instead of writing them to stdout itself -- this function was never the source of
    any of the redraw bugs (full-screen clear, per-line erase, cursor-up+erase-to-end, alt-screen
    buffer, all four tried and replaced in turn); it's pure content generation. The bugs were
    always in the OUTPUT/redraw mechanism below, which is what rich.live.Live now owns instead."""
    mode = CURRENT_MODE
...
# Everything below replaces what used to be hand-rolled ANSI redraw logic (four different attempts
# in the pre-rich version: a full-screen clear, a per-line cursor-up erase, a cursor-up +
# erase-to-end-of-screen, and finally a manual alternate-screen-buffer switch -- each one fixing
# one real, confirmed-live bug and then hitting another from a different angle, including a
# separate line-count-undershoot bug on the CLIENT's own equivalent board). rich.live.Live is
# built exactly for "redraw a live region in place": it diffs the actual rendered output on every
# update() call instead of anyone hand-tracking how many lines the last frame was, which is what
# every one of those bugs ultimately came down to. screen=True uses the terminal's alternate
# screen buffer internally (the same fix already manually implemented and confirmed working for
# the scrollback-pollution complaint -- rich just does it for us now, and correctly restores the
# normal screen on its own context-manager exit instead of needing a separate atexit hook).
_console = Console()

def _dashboard_renderable():
    # Text.from_ansi() lets rich correctly measure lines that already contain raw ANSI color
    # codes (every line _build_dashboard_lines() produces does, via the existing Colors class and
    # _user_color() -- deliberately left untouched, see _build_dashboard_lines's comment) instead
    # of rich trying to interpret escape bytes as literal printable characters. no_wrap=True is
    # required, not cosmetic: these lines are pre-formatted box-drawing art already sized to
    # _build_dashboard_lines()'s own term_width, not flowable paragraph text -- confirmed live
    # that without it, rich wraps a long line (e.g. the "════..." header, or a "┌─ user ── ONLINE
    # ─┐" panel border) across two terminal lines the instant its OWN detected console width
    # disagrees with term_width even slightly, mangling the box structure. Cropping instead of
    # wrapping is the safe failure mode if the two widths are ever out of sync.
    return Group(*(Text.from_ansi(line, no_wrap=True, overflow="crop") for line in _build_dashboard_lines()))

def _dashboard_loop():
    global _DASHBOARD_ACTIVE
    _DASHBOARD_ACTIVE = True
    with Live(_dashboard_renderable(), console=_console, screen=True, auto_refresh=False, transient=False) as live:
        while True:
            try:
                # refresh=True is NOT optional here -- Live.update()'s own `refresh` kwarg
                # defaults to False independent of the constructor's auto_refresh setting (that
                # one only controls whether Live spawns ITS OWN background timer thread; it has
                # no effect on what a manual update() call does). Without this, every update()
                # call here silently updated Live's internal state without ever actually
                # redrawing the terminal -- confirmed live as the real cause of "the dashboard
                # shows the first frame and then just never updates again," not a terminal
                # compatibility issue. A real, confirmed gap in this file's own test coverage:
                # earlier verification checked content correctness and that the Live lifecycle
                # didn't crash, but never asserted that two separate update() calls actually
                # produced two DIFFERENT rendered frames on screen.
                live.update(_dashboard_renderable(), refresh=True)
            except Exception as e:
                # Never let a rendering bug take down the actual server -- try again next cycle.
                # Written with plain print() (not through `live`) since if the renderable itself
                # is what's broken, asking Live to render an error about that would just repeat
                # the same failure.
                print(f"[X] Dashboard render error: {e}")
            # 1s, not the old version's 2s -- confirmed live as noticeably laggy ("takes a little
            # bit before it updates/refreshes"). rich's Live diffs the rendered output before
            # writing anything, so a faster cadence here is cheap -- it's not redrawing the whole
            # screen from scratch every cycle the way the old hand-rolled version effectively was.
            time.sleep(1)

def start_dashboard_if_tty():
    """Only worth running against a real terminal -- a piped/redirected stdout (log file, systemd
    journal) should keep getting plain sequential log_event() lines instead of a live-redrawing
    display, which would make a log file unreadable."""
    if not _TTY:
        return
