...
if sys.platform != "darwin":
    print("[X] This script is explicitly configured for macOS systems only.")
    sys.exit(1)
...
def get_vram_and_choose_model() -> tuple:
    ...
    if has_apple_gpu:
        ...
        gpu_type, total_vram_gb = ("intel_dgpu", dgpu_vram) if has_dgpu else ("cpu", 0.0)
    ...
    elif total_vram_gb >= 3.5 or ram_gb >= 8.0:
...
def main():
    ...
    print("=== Cubyz Dataset Distributed Folding Node (macOS Edition) ===")
    ...
    if gpu_type != "cpu" and total_vram_gb > 8.0:
        mode_desc = "Performance Profile (Automated: High-memory Apple Silicon or discrete GPU detected)"
        cooldown, max_threads = 0.0, None
    elif gpu_type != "cpu" and total_vram_gb <= 8.0:
        mode_desc = "Balanced Profile (Automated: Standard Apple Silicon or discrete GPU detected)"
    ...
    def print_terminal_status(task, step_msg, is_first, comp, tot):
        ...
        if gpu_type == "apple_silicon":
            hw_desc = f"Apple Silicon ({total_vram_gb:.1f} GB effective unified memory)"
        elif gpu_type == "intel_dgpu":
            hw_desc = f"Intel Mac + Discrete GPU ({total_vram_gb:.1f} GB VRAM)"
        else:
            hw_desc = f"CPU Engine ({get_system_ram_gb():.1f} GB RAM)"
