...
PAIRS_DIR = "pipeline/finetune/pairs"
...
def _parse_kb_md_record(collection: str, chunk_id: str) -> dict:
    """Mirrors pipeline/server.py's _parse_kb_md_record() -- kept as a separate
...
def main():
    ...
    os.makedirs("pipeline/finetune/output", exist_ok=True)
    with open("pipeline/finetune/output/audit_report.json", "w", encoding="utf-8") as f:
