...
# Anchor paths to pipeline/ (three levels up: training/ -> finetune/ -> pipeline/) rather than
# the process's cwd -- these scripts get run from both the repo root and from inside
...
# same 152-question benchmark (see archive/finetune_training_diagnostics/compare_models.py), 0.6B only reached 44.6%
...
TRAIN_FILE = os.path.join(PIPELINE_ROOT, "finetune/training/data/train.jsonl")
VAL_FILE = os.path.join(PIPELINE_ROOT, "finetune/training/data/val.jsonl")
OUTPUT_DIR = os.path.join(PIPELINE_ROOT, "finetune/training/output/cubyz-qlora")
