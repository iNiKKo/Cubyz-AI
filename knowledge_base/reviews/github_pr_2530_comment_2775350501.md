...
- **Permissions Struct**: The `Permissions` struct contains an arena allocator, a white list (permissionWhiteList), and a black list (permissionBlackList). Both lists are implemented as string hash maps.

- **Initialization and Deinitialization**: The reviewer suggests that any structure with a deinit function should also have an init function to ensure proper resource management. Specifically, the arena allocator needs to be initialized before use.
