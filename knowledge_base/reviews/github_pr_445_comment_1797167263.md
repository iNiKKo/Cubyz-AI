...
The `taskType` field specifies the type of task being performed, and there are a total of 5 task types (`taskTypes`). These task types are mapped to indices as follows: 0 for `.misc`, 1 for `.io`, 2 for `.compute`, 3 for `.network`, and 4 for `.render`. The manual copying is used because returning `self.*` would not copy the mutex, which needs to be initialized separately. The mutex ensures that only one thread can access the performance metrics at a time, maintaining thread safety.
