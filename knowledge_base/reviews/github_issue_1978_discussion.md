...
## Explanation
To reproduce the issue, hold down the control key while attempting to move; everything is delayed. The delay varies from 200ms to 2000ms and accumulates over time. The maintainer initially suspects it might be related to frame rate or user-specific settings but is unable to reproduce the problem. The user reports that FPS are unlimited and default settings, with a delay ranging from 200ms to 2000ms that accumulates over time. The user also mentions similar issues in Minecraft Java, suggesting a potential broader problem with input processing. However, the maintainer concludes that they cannot fix this issue as it appears to be an issue on the user's side.
