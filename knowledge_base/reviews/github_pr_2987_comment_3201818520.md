...
## Explanation
The code modification adds a new variable `selectionCapabilities` and initializes it to `null`. However, the reviewer points out that the current approach lacks an extra constant at the callsite, which could lead to architectural issues. The reviewer recommends using a struct with a struct-local constant and associated load and check functions to improve the design and prevent potential bugs.
