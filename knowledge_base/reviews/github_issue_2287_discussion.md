...
## Explanation
**Explanation**
The issue proposes adding a timed game exit feature specified by the user, initially intended for CI but with potential other uses. The primary motivation for this is to facilitate Continuous Integration (CI) processes, although QD mentioned other potential uses in comment #2227. The duration for the timed game exit will be specified by the user in some time unit (microseconds or milliseconds, undecided). Once this duration elapses after opening the game (primarily in headless mode but not limited to it), the game will gracefully exit its runtime loop. The maintainer suggests implementing this feature via a command sent to stdin instead of through configuration options.
