...
The `EntityModelIndex` struct contains an `index` field of type `u32`. The `get` method retrieves a pointer to an `EntityModel` instance from the `entityModels.items` array based on the provided index. If the index is within bounds, it returns a pointer to the corresponding `EntityModel`; otherwise, it should always exist because of the first entry in the `entityModelPalette`, which is `cubyz:missing`.

The critical architectural review comment mentions adding a palette loading mechanism with `cubyz:missing` as the 0 entityModel in the future. This ensures that there is always a default model available if an invalid index is accessed.
