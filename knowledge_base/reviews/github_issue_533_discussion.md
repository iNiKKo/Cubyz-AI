...
## Explanation
Discussion on improving gameplay responsiveness during lag by considering multi-threading for physics and networking, with concerns about potential stuttering and performance penalties. The issue revolves around Cubyz's sequential execution of physics, networking, and graphics rendering, leading to lag affecting all aspects. Two solutions are proposed: running physics/networking in a separate thread (which might cause its own stuttering and performance penalties) and performing several extra physics ticks between frames in even time intervals (which would require recording historical inputs for a bit). The maintainer suggests that only using an extra thread is feasible due to limitations in the second option requiring precise timestamps for input events. Specifically, glfwPollEvents must be called from its own thread rather than the render thread to process events accurately. The user highlights specific scenarios where low FPS causes gameplay issues, such as skipping over gaps or failing to register jump inputs when pressing space and releasing quickly. There's a suggestion to extrapolate current inputs over the last frame duration with more granular timesteps but this would require changes in how GLFW event callbacks are processed, including taking timestamps during glfw events callbacks.
