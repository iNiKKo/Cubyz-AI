...
## Explanation
The refactoring simplifies the `getChild` function by leveraging the new `getChildOrNull` method, which returns an optional `ZonElement`. This change enhances readability and reduces redundancy. The reviewer suggests that using `orelse .null` in both functions can further streamline the code while maintaining its functionality.
