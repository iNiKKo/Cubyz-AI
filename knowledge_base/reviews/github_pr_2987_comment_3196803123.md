...
## Explanation
A new variable `selectionCapabilities` is introduced in the code and is initially set to `null`. The reviewer suggests that setting the default selection rule to `.all` when unspecified might be clearer. However, they also point out that allowing `.all` in the configuration could be redundant. The architectural concern here is ensuring that the default behavior is intuitive and that configurations do not introduce unnecessary complexity. Before this change, the default selection rule was set to `.always`. The variable `selectionCapabilities` is intended to provide more granular control over selection rules, potentially replacing the previous single `SelectionRule`. The reviewer's suggestion aims to simplify the configuration by making `.all` the default when no specific rule is provided, thus avoiding redundant entries in the configuration files.
