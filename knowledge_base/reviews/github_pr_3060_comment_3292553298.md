...
## Explanation
The change refactors the `SelectionCapabilities` structure from using an enum of capabilities to a union(enum) that includes a custom struct. This new approach aims to provide more flexibility and efficiency in handling different selection capabilities. The reviewer notes that the previous method was redundant because coercion from enum literals to void union fields is possible, suggesting that the new design simplifies the codebase while maintaining functionality.
