...
## Explanation
The issue discusses a bug where opening a crafting table does not guarantee that the inventory window is open, making it useless until the player manually opens their inventory. The discussion includes suggestions to fix this by ensuring the inventory is always part of the HUD and using flags in the Window struct. Specifically, the chest block does not take a .gui; instead, it has a struct within block_entity.zig that explicitly opens its gui. The maintainer suggests that the inventory should always be open in the menu, specifically as part of the HUD. A flag in the Window struct is mentioned as a potential solution to ensure this behavior. This flag can be used to manage inventory visibility by setting it to true when the inventory should be open and false otherwise. This ensures that the inventory is always part of the HUD and is clickable. The discussion also touches on similar issues with chests and proposes solutions involving changes to block entities and GUI handling.
