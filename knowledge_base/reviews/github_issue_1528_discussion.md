...
## Explanation
The issue discusses the development of a Comptime Modding Framework for Cubyz, including automatic loading of moddable interfaces, creation of a registry for trusted mods, and documentation on modding processes. The main points include automatically loading lists of all moddable interfaces from a common (`mods`) folder, creating a registry to manage trusted mods with metadata such as name, author, link, and secure hash of the source code, and developing a launcher that downloads and compiles mods from the registry when entering the world or joining a server (requires #1483). The maintainers highlight the distinction between trivially moddable systems (like rotations, block entities, entity components, entity systems, UI, simple structures, server commands) and non-trivially moddable systems (like items/tools, rendering), suggesting that non-trivial changes should be tracked separately. There is also discussion about whether to create a registry system from scratch or base it on existing protocols like git.
