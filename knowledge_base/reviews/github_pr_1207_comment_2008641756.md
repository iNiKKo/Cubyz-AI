...
The `BlueprintEntry` struct contains a blueprint and its associated information. The `blueprint` field holds the actual blueprint data, while the `info` field is an instance of the `Info` struct that includes detailed information about the structure. The `Info` struct has two main fields: `originBlock`, which is a `StructureBlock` representing the origin block of the structure, and `childrenBlocks`, which is a list of `StructureBlock` instances representing the child blocks.
