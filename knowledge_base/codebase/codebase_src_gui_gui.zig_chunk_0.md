...
Key functionalities include adding windows to the list, executing open and close commands, and updating window positions. The `GuiCommandQueue` struct has an `Action` enum with `open` and `close` actions, and a `Command` struct that includes a window pointer and an action. The `init` function initializes the command queue with a capacity of 16 using `main.globalAllocator`. The `deinit` function deinitializes the command queue. The `scheduleCommand` function pushes a command into the queue, and the `executeCommands` function processes commands by popping them from the queue and executing the corresponding actions (`open` or `close`). The `executeOpenWindowCommand` function handles opening a window by reordering it in the open windows list and calling its `onOpenFn`. The `executeCloseWindowCommand` function handles closing a window by removing it from the open windows list and calling its `onCloseFn`.
