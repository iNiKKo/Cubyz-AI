...
The `init` method initializes the buffer by allocating memory for the elements using the provided allocator. The `deinit` method deallocates the memory when the buffer is no longer needed. The `pushBack` method adds an element to the end of the buffer if there is space available, otherwise it returns an error. The `forcePushBack` method adds an element to the end of the buffer, removing the front element if the buffer is full, and returns the removed element. The `pushFront` method adds an element to the beginning of the buffer if there is space available, otherwise it returns an error. The `forcePushFront` method adds an element to the beginning of the buffer, removing the back element if the buffer is full, and returns the removed element. The `pushBackSlice` method adds a slice of elements to the end of the buffer, returning an error if there is not enough space. The `insertSliceAtOffset` method inserts a slice of elements at a specific offset in the buffer, returning an error if there is not enough space. The `popBack` method removes and returns the last element from the buffer, or null if the buffer is empty. The `popFront` method removes and returns the first element from the buffer, or null if the buffer is empty. The `popSliceFront` method removes a slice of elements from the front of the buffer, returning an error if the requested number of elements exceeds the current length of the buffer. The `discardElementsFront` method discards a specified number of elements from the front of the buffer. The `getAtOffset` method returns the element at a specific offset in the buffer, or null if the offset is out of bounds.
