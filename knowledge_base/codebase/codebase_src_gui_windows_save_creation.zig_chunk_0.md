...
## Explanation
This chunk manages the creation of a new world through a graphical user interface (GUI). It initializes various GUI components such as text inputs, buttons, and checkboxes to collect user input for world settings like name, seed, gamemode, and presets. The `chooseSeed` function determines the seed value based on user input or generates a random one using Wyhash hashing if parsing fails. Callback functions handle changes in gamemode selection, preset selection, and cheat/testing mode toggles. The `createWorld` function processes the collected data to create a new world using the server's world creation API. The `onOpen` method initializes the GUI window with all necessary components and sets up their interactions. Specifically, it uses padding of 8 units, text inputs for name (128x22 pixels) and seed (128-48x22 pixels), buttons for gamemode selection (128 pixels wide) and preset management (128 pixels wide), checkboxes for allowing cheats and testing mode (only visible in non-tagged releases), and a vertical list to arrange these components. Presets are loaded from `main.assets.worldPresets()` and sorted alphabetically before being displayed. The default preset is selected based on the presence of 'cubyz:default' in the presets, and the seed input field allows users to enter or generate a seed value for the new world.
