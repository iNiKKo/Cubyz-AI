...
The dimensions of the world framebuffer are set to match the window size (`Window.width`, `Window.height`). The reflection cube map is generated by drawing a rectangle across each face of the cube map with a size of 64x64 pixels. During the renderer's shutdown process, various components such as pipelines, framebuffers, and textures are deinitialized.
