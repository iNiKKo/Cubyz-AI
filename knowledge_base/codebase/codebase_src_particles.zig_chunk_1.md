...
## Explanation
The `ParticleManager` struct is responsible for managing particle types and their associated textures. It includes several fields such as `particleTypesSSBO`, `types`, `typesLocal`, `textures`, `emissionTextures`, `textureArray`, `emissionTextureArray`, `ParticleIndex`, and `particleTypeHashmap`. The methods of the `ParticleManager` struct include `init`, `deinit`, `reset`, `register`, `readTextureDataAndParticleType`, `readTexture`, `createAnimationFrames`, `extractAnimationSlice`, and `generateTextureArray`. The `init` method initializes the texture arrays, SSBO, and particle system. The `deinit` method deinitializes these components. The `reset` method resets the particle types and textures. The `register` method registers new particle types by reading texture data and parsing configuration from a ZonElement. Errors are logged if required fields are missing or if texture dimensions are incorrect. The `readTextureDataAndParticleType` function reads base and emission textures, checks for errors, and creates animation frames. The `generateTextureArray` method generates texture arrays for both base and emission textures and binds the particle types SSBO.
