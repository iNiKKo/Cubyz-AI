...
## Explanation
The SwapChain struct is responsible for handling the lifecycle of a Vulkan swap chain. It includes methods to initialize and deinitialize the swap chain, choose appropriate formats, present modes, and extents based on device capabilities. The init method sets up the swap chain by querying supported formats and present modes, selecting optimal values, and creating image views for each swap chain image. The deinit method cleans up resources by destroying image views and the swap chain itself.
