...
## Summary
Handles chunk meshing and rendering preparation for opaque and transparent voxels.

## Explanation
This chunk manages the preparation of chunk meshes for rendering, including both opaque and transparent voxel data. It includes functions to prepare rendering by adding chunks to appropriate lists based on their LOD (Level of Detail) and updating transparency data after mesh uploads. The `prepareRendering` function adds opaque chunks to the render list if they have vertices. The `updateTransparencyDataAfterMeshUpload` function updates sorting data for transparent faces, ensuring they are rendered correctly relative to the player's position. The `prepareTransparentRendering` function checks for changes in transparent data and updates sorting accordingly, using a bucket sort algorithm to organize faces by distance.
