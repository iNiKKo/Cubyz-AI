...
## Explanation
The TextRendering struct manages text rendering using FreeType and HarfBuzz for font management and glyph rendering. The init function initializes a graphics pipeline with specific shaders located at 'assets/cubyz/shaders/graphics/Text.vert' and 'assets/cubyz/shaders/graphics/Text.frag'. It sets up texture parameters such as minification and magnification filters to NEAREST, and wrap modes to REPEAT. The init function also loads a font face from 'assets/cubyz/fonts/unscii-16-full.ttf', creates HarfBuzz objects, and initializes glyph storage with a texture width of 1024 and height of 16. The resizeTexture function adjusts the texture size when needed, copying existing data to a new texture. The uploadData function uploads bitmap data to the texture. The getGlyph function retrieves or loads a glyph's information, rendering it if necessary. The drawGlyph function draws a single glyph at specified coordinates with optional font effects such as bold. The renderText function processes and renders a string of text using these components.
