...
The `discoverIpAddress` function makes the server online and retrieves the external IP address using `main.server.connectionManager.externalAddress`, then formats it as a string and stores it in `ipAddress`. The formatting is done using `std.fmt.allocPrint(main.globalAllocator.allocator, "{f}", .{main.server.connectionManager.externalAddress}) catch unreachable;`. The `invite` function handles user invitations by connecting to the entered IP address. The `copyIp` function copies the IP address to the clipboard. The `onOpen` function initializes the GUI components, including labels, text inputs, and buttons, and starts a thread to discover the IP address. The `onClose` function joins any running threads, frees allocated memory for the IP address string, and deinitializes the GUI components. The `update` function updates the displayed IP address in the GUI if it has changed.
...
Memory management includes freeing allocated memory for the IP address string upon window closure.
