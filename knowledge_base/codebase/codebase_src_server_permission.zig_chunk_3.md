...
- **`permissionListToFromZon` Test**: This test verifies the serialization and deserialization of permission lists using the Zon format. It initializes a `Permissions` object, adds two permissions (`/command/test` and `/command/spawn`), converts the list to a Zon element, and then reconstructs the permissions from the Zon element. The test checks that the reconstructed list contains the same number of permissions as the original.

- **`permissionGroupToFromZon` Test**: This test checks the serialization and deserialization of entire permission groups using the Zon format. It initializes a group named 'test', adds two permissions, serializes the group to a Zon element, and then reconstructs the group from the Zon element. The test verifies that the reconstructed group contains the same number of permissions as the original.
